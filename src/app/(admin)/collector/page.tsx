...
import CollectorDetailDialog from './CollectorDetailDialog';
...
export default function CollectorPage() {
    ...
    const openDetail = (collector: any) => {
        ...
        <main className="w-full max-w-[1440px] mx-auto">
            ...
            <section className="bg-primary-elevated/70 border border-[#dedede10] rounded-3xl p-6 sm:p-8">
                <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 mb-8">
                    <div>
                        <h2 className="text-xl sm:text-2xl font-bold text-text-primary">Collector List</h2>
                        <p className="text-sm text-text-placeholder mt-1">
                            Page {page} of {resolvedTotalPages} • Total {total || 0} collectors
                        </p>
                    </div>
                    ...
                    <table className="w-full min-w-[1000px] text-left text-sm text-text-placeholder border-collapse">
                        ...
                        <tbody className="divide-y divide-[#dedede10]">
                            ...
                            {collectors.map((collector) => (
                                <tr key={collector.id} className="text-text-primary hover:bg-white/5 transition-colors">
                                    <td className="px-6 py-4 font-mono text-xs whitespace-nowrap">{collector.nik}</td>
                                    <td className="px-6 py-4 font-bold whitespace-nowrap">{collector.name}</td>
                                    <td className="px-4 py-4 font-mono text-[10px] text-text-placeholder whitespace-nowrap">
                                        {collector.userId?.substring(0, 12)}...
                                    </td>
                                    <td className="px-6 py-4 max-w-[200px] truncate" title={collector.address}>{collector.address}</td>
                                    <td className="px-6 py-4 text-center">
                                        <span
                                            className={`inline-flex items-center gap-2 rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-widest whitespace-nowrap ${collector.deleted
            ...
            <CollectorDetailDialog
                isOpen={isDetailOpen}
                onClose={() => { setIsDetailOpen(false); setSelectedCollector(null); }}
                collector={selectedCollector}
            />
        </main>
    );
